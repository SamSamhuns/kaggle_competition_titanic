...
def plot_boundary_line(theta_vector, X_feat, y_target):
    ...
    x_min, x_max, y_min, y_max = min(X_feat[:, 1]) - 0.5, max(X_feat[:, 1]) + 0.5, min(
        X_feat[:, 2]) - 0.5, max(X_feat[:, 2]) + 0.5  # find min,max of both x and y axis values

    # gen evenly spaced 100 points between min, max values in the x and y axis
    x0, y0 = np.linspace(x_min, x_max, num=100), np.linspace(
        y_min, y_max, num=100)
    ...
    '''
    X_all = np.c_[np.ones((xx.ravel().shape[0], 1)), xx.ravel(
    ), yy.ravel()]  # for just the linear logistic regression part
    if theta_vector.size == 6:  # 2nd degree polynomial logistic regression
        ...
        eng_feat2 = np.array(X_all[:, 1] * X_all[:, 2]
                             ).reshape(X_all.shape[0], 1)  # ab
        # np.concatenate((X_feat, eng_feat1, eng_feat2), axis=1)
        X_all = np.c_[X_all, eng_feat1, eng_feat2]
    ...
    # plot the contours
    plt.contourf(xx, yy, X_all_pred,
                 cmap=mpl.colors.ListedColormap(['#cc9880', '#7e97cc']))
...
def print_accuracy_best_get_parameters(X_feat, y_target, regr_model):
    ...
    parameters = regr_model.theta_vector.reshape(
        1, regr_model.theta_vector.shape[0])
    print(
        f"Parameterx for the model = {parameters}, \nloss = {np.asscalar(regr_model.loss[-1])}, \naccuracy = {logistic_accuracy(pred, y_target)}")
